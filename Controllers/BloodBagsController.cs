...
namespace BD_26.Controllers
{
    public class BloodBagsController : Controller
    {
        ...
        public ActionResult Index(string sortOrder, string searchString)
        {
            ...
            bloodBags = from s in db.BloodBags
                                  select s;
            if (!String.IsNullOrEmpty(searchString))
            {
                bloodBags = bloodBags.Where(s => s.BloodBank.bank_name.ToUpper().Contains(searchString.ToUpper())
            ...
            {
                case "name_desc":
                    bloodBags = bloodBags.OrderByDescending(s => s.BloodBank.bank_name);
                    break;
                case "name_asc":
                    bloodBags = bloodBags.OrderBy(s => s.BloodBank.bank_name);
                    break;

                default:
                    bloodBags = bloodBags.OrderBy(s => s.bloodtype);
                    break;
            }
